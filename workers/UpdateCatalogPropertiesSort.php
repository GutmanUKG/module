...
class UpdateCatalogPropertiesSort {
    ...
    public function __construct () {
        $this->Logger = new ImarketLogger("/upload/log/UpdateCatalogPropertiesSort/");
    ...
    private function updateCategoryPropertiesName(): void
    {
        ...
        $siteProperties = [];
        $sitePropertiesSort = [];
        $sql = "SELECT ID, NAME, CODE, SORT FROM b_iblock_property WHERE IBLOCK_ID = {$this->catalogIblockId}";
        $res = $this->connection->query($sql);
        while($arItem = $res->fetch()) {
            $siteProperties[$arItem["CODE"]] = $arItem;
            $sitePropertiesSort[$arItem['ID']] = $arItem['SORT'];
        }

//        file_put_contents($_SERVER["DOCUMENT_ROOT"]."/upload/catalog_properties.txt", print_r($siteProperties, true));
//        file_put_contents($_SERVER["DOCUMENT_ROOT"]."/upload/catalog_properties_sort.txt", print_r($sitePropertiesSort, true));
...
//            }
            ...
            if (!empty($catalogAppSection["PROPERTIES"])) {
                foreach ($catalogAppSection["PROPERTIES"] as $CAProperty) {
                    ...
                    $sql = "UPDATE b_iblock_property SET `SORT` = '{$CAProperty['order']}' WHERE CODE = '{$siteProperties[$propCode]['CODE']}'";
...
(new UpdateCatalogPropertiesSort())->StartWorker();