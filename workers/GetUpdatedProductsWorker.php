...
class GetUpdatedProductsWorker {
...
//    private $useDescriptionForPropData = true;
    ...
    public function __construct () {
        $this->Logger = new ImarketLogger("/upload/log/GetUpdatedProductsWorker/");
...
//        $this->UpdateStatus(1);
        ...
        $catalogAppProductIds = array_keys($this->arModelsByXML_ID);
        $siteProductIds = [];

        $this->Logger->log("LOG", "Получаем товары для простановки флага");
//        $sql = "SELECT IBLOCK_ELEMENT_ID FROM b_iblock_element_property WHERE IBLOCK_PROPERTY_ID = 12443 AND VALUE IN (".implode(',', $catalogAppProductIds).")";
        $sql = "SELECT ID FROM b_iblock_element WHERE IBLOCK_ID = {$this->catalogIblockId} AND XML_ID IN ('".implode("','", $catalogAppProductIds)."')";
        $res = $this->connection->query($sql);
        while ($row = $res->fetch()) {
            $siteProductIds[] = $row['ID'];
        }

        $this->Logger->log("LOG", "Получено товаров ".count($siteProductIds));

        $this->Logger->log("LOG", "Обновляем флаг");
        $sql = "UPDATE b_iblock_element_property SET VALUE = 0, VALUE_NUM = 0 WHERE IBLOCK_PROPERTY_ID = 56563 AND IBLOCK_ELEMENT_ID IN (".implode(',', $siteProductIds).")";
...
//        $this->arModels = array_reverse($this->arModels, true);
        ...
        $date = date("Y-m-d", strtotime("-1 day"))."T00:00:00";
...
(new GetUpdatedProductsWorker())->StartWorker();